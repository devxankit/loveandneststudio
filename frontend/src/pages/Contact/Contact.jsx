...
const Contact = () => {
    ...
    // Dynamic Images
    const entranceImage = sidebarSection.entranceImage || img1;
    const verticalImage = sidebarSection.verticalImage || verticalImg;
    const collageImages = collageSection.images?.length > 0 ? collageSection.images : [img2, img3];

    return (
        <>
            <SEO
                title="Connect | Love & Nest Studio"
                description={heroSection.text?.substring(0, 150) || "Reach out to Love & Nest Studio..."}
                    ...
                    <div className="flex flex-col lg:flex-row rounded-[40px] md:rounded-[60px] overflow-hidden shadow-[0_40px_100px_-20px_rgba(90,42,69,0.12)] bg-white/70 backdrop-blur-xl border border-white/40">
                        ...
                        <div className="w-full lg:w-[65%] bg-[#C9D0C3]/40 relative p-8 md:p-16 flex flex-row">
                            ...
                            <div className="flex-1 flex flex-col items-start pl-8 md:pl-16">
                                ...
                                >
                                    <span className="block text-[10px] font-bold uppercase tracking-[0.6em] text-[#B77A8C] mb-6">{heroSection.subheading || "Inquire with Us"}</span>
                                    <h1 className="font-display text-[10vw] lg:text-[6rem] leading-[0.85] text-[#5A2A45] mb-4">
                                        {heroSection.heading ? (
                                            <span dangerouslySetInnerHTML={{ __html: heroSection.heading.replace('Connect', '<span class="italic font-light opacity-40 font-serif">Connect</span>') }} />
                                        ) : (
                                    ...
                                    <p className="font-outfit text-[#6E5A52] text-base md:text-lg leading-relaxed max-w-sm mt-8 opacity-80">
                                        "{heroSection.text || "Because every breath, every giggle, and every tiny finger deserves to be remembered exactly as it felt."}"
                                    </p>
                                </motion.div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 w-full max-w-2xl">
                                    <ContactRow
                                        ...
                                        text={infoSection.email || "loveandnest@gmail.com"}
                                        href={`mailto:${infoSection.email || "loveandnest@gmail.com"}`}
                                    />
                                    <ContactRow
                                        delay={0.5}
                                        ...
                                        text={infoSection.phone || "+91 86790 76776"}
                                        href={`tel:${infoSection.phone ? infoSection.phone.replace(/ /g, '') : "+918679076776"}`}
                                    />
                                    <ContactRow
                                        delay={0.6}
                                        ...
                                        text={infoSection.location || "Dehradun, Uttarakhand"}
                                    />
                                </div>
                                ...
                                >
                                    ...
                                    <span className="block mt-4 text-[10px] font-bold uppercase tracking-[0.4em] text-[#5A2A45]/30">{sidebarSection.entranceLabel || "Studio Entrance — Dehradun"}</span>
                                </motion.div>
                            </div>
                        </div>
