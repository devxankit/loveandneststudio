...
const Baby = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await getBabyPage();
                setData(res.data);
            } catch (error) {
                console.error("Failed to fetch baby page data", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const puzzleLayout = [
        ...
        ...slot,
        img: puzzleImages[i % puzzleImages.length] || '' // Cycle through images or empty
    }));

    const InfiniteColumn = ({ speed, images }) => {
        if (!images || images.length === 0) return <div className="w-full h-full bg-[#FAFAFA]" />;
        const displayImages = [...images, ...images, ...images]; // Duplicate for scroll length
        return (
                ...
                >
                    {displayImages.map((img, i) => (
                        <div key={i} className="relative group w-full p-2">
                            <div className="absolute inset-0 transform rotate-2 rounded-xl" style={{ backgroundColor: i % 2 === 0 ? '#F3EFE0' : '#E8E4D9' }}></div>
                            <div className="relative overflow-hidden rounded-lg shadow-sm transform -rotate-1 hover:rotate-0 transition-transform duration-500">
                                <LazyImage src={img} className="w-full aspect-[3/4] object-cover hover:scale-105 transition-transform duration-700" />
                            </div>
                        </div>
                    ))}
                </motion.div>
    ...
    return (
        <>
            <SEO
                title={`${hero.title || 'Baby Photography'} | Love & Nest Studio`}
                description={hero.text || "Adorable baby photography capturing growing milestones."}
            ...
            <div className="relative min-h-screen bg-[#FDFBF7] font-outfit text-[#4A4A4A] overflow-x-hidden pb-32">
                ...
                <div className="absolute top-0 left-0 right-0 h-[75vh] overflow-hidden flex gap-4 md:gap-8 justify-center px-2 md:px-0 z-0">
                    <div className="hidden md:block w-1/4 pt-10"><InfiniteColumn speed={35} images={bgImages} /></div>
                    <div className="w-1/2 md:w-1/4 pt-0"><InfiniteColumn speed={45} images={bgImages} /></div>
                    <div className="w-1/2 md:w-1/4 pt-20"><InfiniteColumn speed={30} images={bgImages} /></div>
                    <div className="hidden md:block w-1/4 pt-5"><InfiniteColumn speed={40} images={bgImages} /></div>
                ...
                <section className="relative z-10 min-h-[60vh] flex items-center justify-center pt-10 md:pt-20">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }} className="max-w-2xl mx-6 text-center">
                        <motion.div animate={{ rotate: [0, 5, -5, 0] }} transition={{ duration: 6, repeat: Infinity }}>
                            <span className="block text-sm md:text-base uppercase tracking-[0.4em] mb-4 text-[#B77A8C] font-medium">{hero.subtitle}</span>
                        </motion.div>
                        <h1 className="font-display text-5xl md:text-7xl text-[#4A4A4A] mb-6 leading-tight">
                            {hero.title?.split(' ').map((word, i) => i === 1 ? <span key={i}><br /><span className="italic text-[#B77A8C]">{word}</span></span> : word + ' ')}
                        </h1>
                        <p className="font-outfit font-light text-gray-600 tracking-wide max-w-md mx-auto">{hero.text}</p>
                    </motion.div>
                </section>
                    ...
                    <div className="max-w-6xl mx-auto px-6 h-full flex flex-col md:flex-row items-center gap-16 md:gap-24 relative z-10">
                        ...
                        <div className="w-full md:w-1/2 text-center md:text-left space-y-6">
                            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
                                <span className="font-handwriting text-3xl md:text-5xl text-[#4A4A4A] block mb-2 rotate-[-2deg]">{welcome.handwriting}</span>
                                <h2 className="font-display text-4xl md:text-5xl lg:text-6xl text-[#1a1a1a] leading-tight">{welcome.title}</h2>
                            </motion.div>
                ...
                <section className="relative z-10 px-4 md:px-8 mt-12 md:mt-24">
                    <div className="text-center mb-12"><p className="uppercase tracking-[0.3em] text-xs text-[#B77A8C]">Sneak Peek</p></div>
